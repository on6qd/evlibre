...
class HandleDiagnosticsStatusUseCaseTest {

    @Test
    void logs_diagnostics_status() {
        var eventLog = new RecordingEventLog();
        var useCase = new HandleDiagnosticsStatusUseCase(eventLog);

        useCase.handleDiagnosticsStatus(
                new TenantId("demo-tenant"),
                new ChargePointIdentity("CHARGER-001"),
                "Uploaded");

        assertThat(eventLog.events).hasSize(1);
        assertThat(eventLog.events.get(0)).contains("DiagnosticsStatusNotification");
    }
